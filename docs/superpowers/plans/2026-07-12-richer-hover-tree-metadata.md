...
**Goal:** Present owner, lifecycle state, and optional trusted Git attribution consistently in AnnoPulse Hovers and Explorer items without changing annotation membership or triggering Git work by default.
...
- Add `annopulse.git.showMetadata` as a boolean configuration with default `false`.
...
- `src/composables/use-annotation-explorer.ts`: exposes the existing metadata index to the Tree provider and gates batch hydration on the new setting.
...
- `tests/annotation-explorer.test.ts`: metadata-hydration policy and refresh coverage.
...
- Consumes: `AnnoPulseAnnotation` from `src/types/annotation.ts` and optional `AnnoPulseGitMetadata` from `src/core/git/blame.ts`.
- Produces: `annotationDisplayOwner`, `annotationDisplayState`, `formatAnnoPulseGitAge`, `formatAnnoPulseExplorerDescription`, and `formatAnnoPulseExplorerTooltip` for later Tree use.
...
```ts
expect(annotationDisplayOwner(annotation({ owner: '  Ada  ' }))).toBe('Ada')
expect(annotationDisplayOwner(annotation({ owner: '   ' }))).toBeUndefined()
expect(annotationDisplayState(annotation())).toBe('active')
expect(annotationDisplayState(annotation({ resolved: true }))).toBe('resolved')
expect(annotationDisplayState(annotation({ ignored: true }))).toBe('ignored')
expect(
  annotationDisplayState(annotation({ ignored: true, resolved: true })),
...
```ts
expect(formatAnnoPulseExplorerDescription(annotation, metadata, now)).toBe(
  '2:4 • @Ada • Grace Hopper • 1 day ago • resolved',
)
expect(formatAnnoPulseExplorerTooltip(annotation, metadata, now)).toContain(
  'Owner: @Ada',
)
expect(formatAnnoPulseExplorerTooltip(annotation, metadata, now)).toContain(
  'State: resolved',
...
```ts
export function annotationDisplayOwner(
  annotation: AnnoPulseAnnotation,
): string | undefined

export function annotationDisplayState(annotation: AnnoPulseAnnotation): string
...
Implement the helpers with no VS Code import. `annotationDisplayOwner` must call `annotation.owner?.trim()` and return `undefined` for the empty result. `annotationDisplayState` must return the four exact strings asserted in Step 1. `formatAnnoPulseGitAge` must parse `metadata?.commitDate`, return `undefined` for non-finite values, return `today` for a non-positive day difference, and otherwise return singular/plural day strings using `Math.floor((now.getTime() - commitTime) / 86_400_000)`.

`formatAnnoPulseExplorerDescription` must build `[location, owner, author, age, resolved, ignored]`, omitting absent owner/author/age and including resolved/ignored as separate segments. `formatAnnoPulseExplorerTooltip` must build newline-separated plain text in this exact order: title, `Location`, `Owner`, `State`, then (only when metadata exists) a `Git:` section followed by `Author`, optional `Email`, `Date`, optional `Age`, `Commit` with `hash.slice(0, 7)`, and `Summary`.
...
Change `formatAnnoPulseHoverMarkdown` to accept an optional third `now: Date = new Date()` parameter. Import `annotationDisplayOwner`, `annotationDisplayState`, and `formatAnnoPulseGitAge`.
...
Use `escapeMarkdown` for annotation keyword/message, owner, category, severity, rule id, source, URI, Git author/email/hash/summary, and the date value. Keep labels and Markdown list syntax literal. Add `Owner` only when `annotationDisplayOwner` returns a value and always add `State`. In the Git section add optional Email and Age in the exact order tested in Step 4. Preserve the existing category/severity/rule/source/location/Git fields.
...
rtk git commit -m "feat: enrich annotation metadata presentation"
```
...
- Consumes: Task 1's `formatAnnoPulseExplorerDescription` and `formatAnnoPulseExplorerTooltip` plus `AnnoPulseGitMetadata`.
...
In `tests/package-metadata.test.ts`, insert `'annopulse.git.showMetadata'` immediately after `'annopulse.git.staleDays'` in the strict configuration key list. Add:
...
Add this exact property after `annopulse.git.staleDays` in `package.json`:
...
Extend the constructor after `getGroupBy` with defaulted `getMetadataByAnnotationId: GetAnnoPulseGitMetadata = () => new Map()` and `getNow: GetAnnoPulseNow = () => new Date()`. Store both readers. Convert `createTreeItem` from the current static callback into an instance method so it can read the metadata map and current time. For leaves, obtain `metadata = this.getMetadataByAnnotationId().get(annotation.id)`. Apply the rich description only when metadata is present, preserving the current compact description otherwise. Always use the pure plain-text Tooltip so owner/state are consistently available; it adds Git fields only when metadata is present. Remove the now-unused `formatAnnoPulseLink` import. Keep commands, context values, icons, resource URI, grouping, sorting, and children behavior unchanged.

Use `formatAnnoPulseExplorerDescription(annotation, metadata, this.getNow())` for metadata leaves. For every Tooltip, use `formatAnnoPulseExplorerTooltip(annotation, metadata, this.getNow())`. Do not calculate metadata, mutate maps, or call Git in this provider.
...
- Consumes: `config.git.showMetadata`, the existing `AnnoPulseExplorerGitMetadataIndex`, and Task 2's metadata-aware `AnnoPulseTreeDataProvider`.
...
Pass `() => gitMetadataIndex.metadataByAnnotationId` as the third reader when creating `AnnoPulseTreeDataProvider` in `useAnnoPulseExplorer`.
...
```bash
rtk pnpm vitest tests/annotation-explorer.test.ts tests/tree-data-provider.test.ts tests/hover-format.test.ts tests/git-presentation.test.ts tests/package-metadata.test.ts
rtk pnpm format:check
rtk pnpm lint
pnpm typecheck
...
```bash
rtk git add src/composables/use-annotation-explorer.ts tests/annotation-explorer.test.ts
...
Task 1 defines the five pure presentation exports consumed by Task 2. Task 2 defines the metadata-reader constructor contract consumed by Task 3. Task 3 supplies the existing `ReadonlyMap<string, AnnoPulseGitMetadata>` index, matching the Task 2 reader type.
